...
task('full-deploy-demo-mumbai', 'deploys the entire Lens Protocol').setAction(async ({ }, hre) => {
  ...
  // );
  await waitForTx(
    lensHub.whitelistFollowModule(superFollowModule.address, true, { nonce: governanceNonce++ })
  );
  // --- COMMENTED OUT AS THIS IS NOT A LAUNCH MODULE ---
  // await waitForTx(
  ...
  await waitForTx(
    moduleGlobals
      .connect(governance)
      .whitelistCurrency(paymentToken, true, { nonce: governanceNonce++ })
  );
  ...
  const addrs = {
    ...
    'superFollow': superFollowModule.address,
